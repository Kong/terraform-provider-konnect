#!/usr/bin/env bash
mkdir -p build/complete/go-sdk-internal

export KEEP_SPEAKEASY_ANNOTATIONS=1
export REQUIRE_SPEAKEASY_ENTITY_OPERATION=0
export SDK_NAME=go-sdk-internal

# Get path to current file
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"

# Process specs
$DIR/run.sh go-sdk-internal; 
...
function apply_overlay() {
  set -x
  speakeasy overlay apply -o $DIR/../$1 -s $DIR/../build/complete/go-sdk-internal/public.yaml > $DIR/../build/complete/go-sdk-internal/public2.yaml
  mv $DIR/../build/complete/go-sdk-internal/public2.yaml $DIR/../build/complete/go-sdk-internal/public.yaml
}
...
SDK_KONNECT_GO_DIR="${DIR}/../../sdk-konnect-go-internal"
if [[ ! -d "${SDK_KONNECT_GO_DIR}" ]]
then
  echo "${SDK_KONNECT_GO_DIR} directory not found, skipping copy"
else
  cp $DIR/../build/complete/go-sdk-internal/public.yaml "${SDK_KONNECT_GO_DIR}/openapi.yaml"
fi
