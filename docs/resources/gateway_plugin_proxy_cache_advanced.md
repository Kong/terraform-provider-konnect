...
- `ignore_uri_case` (Boolean) Determines whether to treat URIs as case sensitive. By default, case sensitivity is enabled. If set to true, requests are cached while ignoring case sensitivity in the URI. Default: false
...
- `request_method` (List of String) Downstream request methods considered cacheable. Available options: `HEAD`, `GET`, `POST`, `PATCH`, `PUT`. Default: ["GET","HEAD"]
- `response_code` (List of Number) Upstream response status code considered cacheable. The integers must be a value between 100 and 900. Default: [200,301,404]
...
```shell
terraform import konnect_gateway_plugin_proxy_cache_advanced.my_konnect_gateway_plugin_proxy_cache_advanced '{"control_plane_id": "9524ec7d-36d9-465d-a8c5-83a3c9390458", "id": "3473c251-5b6c-4f45-b1ff-7ede735a366d", "workspace": "team-payments"}'
```
