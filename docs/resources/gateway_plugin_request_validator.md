...
- `enforcement_mode` (String) Determines the action to take when a request fails validation. When set to `block`, the request is rejected with an HTTP 400 response. When set to `log_only`, the request is allowed to proceed and a warning is logged. possible known values include one of ["block", "log_only"]; Default: "block"
