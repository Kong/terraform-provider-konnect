...
resource "konnect_gateway_plugin_confluent" "my_gatewaypluginconfluent" {
  ...
  config = {
    ...
    producer_async_health_failure_threshold            = 3
    producer_async_health_gating                       = false
    producer_async_health_probe_interval               = 5000
    producer_async_health_recovery_threshold           = 1
    producer_config_enabled                            = false
...
- `new_kafka_async_producer` (Boolean) Use the improved asynchronous Kafka producer, which batches messages more efficiently under high load. Only affects asynchronous mode. Messages without a key may be reordered across partitions; set a message key if ordering matters. Default: false
...
- `producer_async` (Boolean) Flag to enable asynchronous mode. Only takes effect when `producer_config_enabled` is true. Default: true
...
- `producer_async_health_failure_threshold` (Number) Number of consecutive authentication/authorization or connectivity failures before the producer is marked unhealthy and async requests are gated. Default: 3
- `producer_async_health_gating` (Boolean) In asynchronous mode, fail incoming requests fast with HTTP 503 (instead of returning 200 and silently dropping the message) when the producer is sustainedly failing to authenticate with, or reach, the broker. Disabled by default. Default: false
- `producer_async_health_probe_interval` (Number) Interval in milliseconds between background recovery probes while the producer is unhealthy. Default: 5000
- `producer_async_health_recovery_threshold` (Number) Number of consecutive successful recovery probes required before the producer resumes accepting async requests. Default: 1
- `producer_config_enabled` (Boolean) Honor the `producer_*` delivery settings (asynchronous mode, request acks/limits/retries). Historically the Confluent plugin ignored these and always produced synchronously; this flag defaults to false to preserve that behavior. Set to true to opt into the configured producer behavior. Default: false
...
- `exclude_headers` (List of String) Blocklist of HTTP header names to exclude from forwarding. Used when `forward_all_by_default` is `enabled`. Default: []
- `forward_all_by_default` (Boolean) When `false`, only headers listed in `include_headers` are forwarded. When `true`, all headers except those in `exclude_headers` are forwarded. Default: false
- `forward_http_headers_as_record_headers` (Boolean) Whether to forward HTTP headers as Kafka record headers. Default: true
- `include_headers` (List of String) Allowlist of HTTP header names to forward as Kafka record headers. Used when `forward_all_by_default` is `disabled`. Default: []
...
- `repeated_headers_behavior` (String) How to handle repeated HTTP headers: `concatenate_by_comma` joins values with a comma, `take_first` uses only the first value, `retain_duplicates` creates separate Kafka record headers for each value. possible known values include one of ["concatenate_by_comma", "retain_duplicates", "take_first"]; Default: "retain_duplicates"
...
- `payload_encoding` (String) How the client encodes union/nullable fields in the request body for Avro schemas. 'avro_json' (default) requires Avro-spec JSON with type-tagged unions (e.g. {"int": 1}, {"null": null}). 'simple_json' lets the gateway accept plain JSON and resolve union branches against the schema. possible known values include one of ["avro_json", "simple_json"]; Default: "avro_json"
