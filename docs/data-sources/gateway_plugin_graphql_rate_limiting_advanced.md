...
page_title: "konnect_gateway_plugin_graphql_rate_limiting_advanced Data Source - terraform-provider-konnect"
subcategory: ""
description: |-
  GatewayPluginGraphqlRateLimitingAdvanced DataSource
---

# konnect_gateway_plugin_graphql_rate_limiting_advanced (Data Source)
...
```terraform
data "konnect_gateway_plugin_graphql_rate_limiting_advanced" "my_gatewayplugingraphqlratelimitingadvanced" {
...
- `cost_strategy` (String) Strategy to use to evaluate query costs. Either `default` or `node_quantifier`.
...
- `identifier` (String) How to define the rate limit key. Can be `ip`, `credential`, `consumer`.
...
- `strategy` (String) The rate-limiting strategy to use for retrieving and incrementing the limits.
...
- `window_type` (String) Sets the time window to either `sliding` or `fixed`.
