...
- `session_redis_connect_timeout` (Number) Session redis connection timeout in milliseconds.
- `session_redis_host` (String) The Redis host.
- `session_redis_password` (String) Password to use for Redis connection when the `redis` session storage is defined. If undefined, no AUTH commands are sent to Redis.
- `session_redis_port` (Number) The Redis port.
- `session_redis_prefix` (String) The Redis session key prefix.
- `session_redis_read_timeout` (Number) Session redis read timeout in milliseconds.
- `session_redis_send_timeout` (Number) Session redis send timeout in milliseconds.
- `session_redis_server_name` (String) The SNI used for connecting the Redis server.
- `session_redis_socket` (String) The Redis unix socket path.
- `session_redis_ssl` (Boolean) Use SSL/TLS for Redis connection.
- `session_redis_ssl_verify` (Boolean) Verify identity provider server certificate.
- `session_redis_username` (String) Username to use for Redis connection when the `redis` session storage is defined and ACL authentication is desired. If undefined, ACL authentication will not be performed. This requires Redis v6.0.0+. To be compatible with Redis v5.x.y, you can set it to `default`.
