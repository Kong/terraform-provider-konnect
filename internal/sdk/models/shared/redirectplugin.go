// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
	"github.com/kong/terraform-provider-konnect/v2/internal/sdk/internal/utils"
)

type RedirectPluginConfig struct {
	// Use the incoming request's path and query string in the redirect URL
	KeepIncomingPath *bool `json:"keep_incoming_path,omitempty"`
	// The URL to redirect to
	Location *string `json:"location,omitempty"`
	// The response code to send. Must be an integer between 100 and 599.
	StatusCode *int64 `json:"status_code,omitempty"`
}

func (o *RedirectPluginConfig) GetKeepIncomingPath() *bool {
	if o == nil {
		return nil
	}
	return o.KeepIncomingPath
}

func (o *RedirectPluginConfig) GetLocation() *string {
	if o == nil {
		return nil
	}
	return o.Location
}

func (o *RedirectPluginConfig) GetStatusCode() *int64 {
	if o == nil {
		return nil
	}
	return o.StatusCode
}

// RedirectPluginConsumer - If set, the plugin will activate only for requests where the specified has been authenticated. (Note that some plugins can not be restricted to consumers this way.). Leave unset for the plugin to activate regardless of the authenticated Consumer.
type RedirectPluginConsumer struct {
	ID *string `json:"id,omitempty"`
}

func (o *RedirectPluginConsumer) GetID() *string {
	if o == nil {
		return nil
	}
	return o.ID
}

// RedirectPluginConsumerGroup - If set, the plugin will activate only for requests where the specified consumer group has been authenticated. (Note that some plugins can not be restricted to consumers groups this way.). Leave unset for the plugin to activate regardless of the authenticated Consumer Groups
type RedirectPluginConsumerGroup struct {
	ID *string `json:"id,omitempty"`
}

func (o *RedirectPluginConsumerGroup) GetID() *string {
	if o == nil {
		return nil
	}
	return o.ID
}

type RedirectPluginProtocols string

const (
	RedirectPluginProtocolsGrpc  RedirectPluginProtocols = "grpc"
	RedirectPluginProtocolsGrpcs RedirectPluginProtocols = "grpcs"
	RedirectPluginProtocolsHTTP  RedirectPluginProtocols = "http"
	RedirectPluginProtocolsHTTPS RedirectPluginProtocols = "https"
)

func (e RedirectPluginProtocols) ToPointer() *RedirectPluginProtocols {
	return &e
}
func (e *RedirectPluginProtocols) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "grpc":
		fallthrough
	case "grpcs":
		fallthrough
	case "http":
		fallthrough
	case "https":
		*e = RedirectPluginProtocols(v)
		return nil
	default:
		return fmt.Errorf("invalid value for RedirectPluginProtocols: %v", v)
	}
}

// RedirectPluginRoute - If set, the plugin will only activate when receiving requests via the specified route. Leave unset for the plugin to activate regardless of the route being used.
type RedirectPluginRoute struct {
	ID *string `json:"id,omitempty"`
}

func (o *RedirectPluginRoute) GetID() *string {
	if o == nil {
		return nil
	}
	return o.ID
}

// RedirectPluginService - If set, the plugin will only activate when receiving requests via one of the routes belonging to the specified Service. Leave unset for the plugin to activate regardless of the Service being matched.
type RedirectPluginService struct {
	ID *string `json:"id,omitempty"`
}

func (o *RedirectPluginService) GetID() *string {
	if o == nil {
		return nil
	}
	return o.ID
}

// RedirectPlugin - A Plugin entity represents a plugin configuration that will be executed during the HTTP request/response lifecycle. It is how you can add functionalities to Services that run behind Kong, like Authentication or Rate Limiting for example. You can find more information about how to install and what values each plugin takes by visiting the [Kong Hub](https://docs.konghq.com/hub/). When adding a Plugin Configuration to a Service, every request made by a client to that Service will run said Plugin. If a Plugin needs to be tuned to different values for some specific Consumers, you can do so by creating a separate plugin instance that specifies both the Service and the Consumer, through the `service` and `consumer` fields.
type RedirectPlugin struct {
	// Unix epoch when the resource was created.
	CreatedAt *int64 `json:"created_at,omitempty"`
	// Whether the plugin is applied.
	Enabled      *bool             `json:"enabled,omitempty"`
	ID           *string           `json:"id,omitempty"`
	InstanceName *string           `json:"instance_name,omitempty"`
	name         string            `const:"redirect" json:"name"`
	Ordering     map[string]string `json:"ordering,omitempty"`
	// An optional set of strings associated with the Plugin for grouping and filtering.
	Tags []string `json:"tags,omitempty"`
	// Unix epoch when the resource was last updated.
	UpdatedAt *int64               `json:"updated_at,omitempty"`
	Config    RedirectPluginConfig `json:"config"`
	// If set, the plugin will activate only for requests where the specified has been authenticated. (Note that some plugins can not be restricted to consumers this way.). Leave unset for the plugin to activate regardless of the authenticated Consumer.
	Consumer *RedirectPluginConsumer `json:"consumer"`
	// If set, the plugin will activate only for requests where the specified consumer group has been authenticated. (Note that some plugins can not be restricted to consumers groups this way.). Leave unset for the plugin to activate regardless of the authenticated Consumer Groups
	ConsumerGroup *RedirectPluginConsumerGroup `json:"consumer_group"`
	// A set of strings representing HTTP protocols.
	Protocols []RedirectPluginProtocols `json:"protocols,omitempty"`
	// If set, the plugin will only activate when receiving requests via the specified route. Leave unset for the plugin to activate regardless of the route being used.
	Route *RedirectPluginRoute `json:"route"`
	// If set, the plugin will only activate when receiving requests via one of the routes belonging to the specified Service. Leave unset for the plugin to activate regardless of the Service being matched.
	Service *RedirectPluginService `json:"service"`
}

func (r RedirectPlugin) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(r, "", false)
}

func (r *RedirectPlugin) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &r, "", false, false); err != nil {
		return err
	}
	return nil
}

func (o *RedirectPlugin) GetCreatedAt() *int64 {
	if o == nil {
		return nil
	}
	return o.CreatedAt
}

func (o *RedirectPlugin) GetEnabled() *bool {
	if o == nil {
		return nil
	}
	return o.Enabled
}

func (o *RedirectPlugin) GetID() *string {
	if o == nil {
		return nil
	}
	return o.ID
}

func (o *RedirectPlugin) GetInstanceName() *string {
	if o == nil {
		return nil
	}
	return o.InstanceName
}

func (o *RedirectPlugin) GetName() string {
	return "redirect"
}

func (o *RedirectPlugin) GetOrdering() map[string]string {
	if o == nil {
		return nil
	}
	return o.Ordering
}

func (o *RedirectPlugin) GetTags() []string {
	if o == nil {
		return nil
	}
	return o.Tags
}

func (o *RedirectPlugin) GetUpdatedAt() *int64 {
	if o == nil {
		return nil
	}
	return o.UpdatedAt
}

func (o *RedirectPlugin) GetConfig() RedirectPluginConfig {
	if o == nil {
		return RedirectPluginConfig{}
	}
	return o.Config
}

func (o *RedirectPlugin) GetConsumer() *RedirectPluginConsumer {
	if o == nil {
		return nil
	}
	return o.Consumer
}

func (o *RedirectPlugin) GetConsumerGroup() *RedirectPluginConsumerGroup {
	if o == nil {
		return nil
	}
	return o.ConsumerGroup
}

func (o *RedirectPlugin) GetProtocols() []RedirectPluginProtocols {
	if o == nil {
		return nil
	}
	return o.Protocols
}

func (o *RedirectPlugin) GetRoute() *RedirectPluginRoute {
	if o == nil {
		return nil
	}
	return o.Route
}

func (o *RedirectPlugin) GetService() *RedirectPluginService {
	if o == nil {
		return nil
	}
	return o.Service
}

// RedirectPluginInput - A Plugin entity represents a plugin configuration that will be executed during the HTTP request/response lifecycle. It is how you can add functionalities to Services that run behind Kong, like Authentication or Rate Limiting for example. You can find more information about how to install and what values each plugin takes by visiting the [Kong Hub](https://docs.konghq.com/hub/). When adding a Plugin Configuration to a Service, every request made by a client to that Service will run said Plugin. If a Plugin needs to be tuned to different values for some specific Consumers, you can do so by creating a separate plugin instance that specifies both the Service and the Consumer, through the `service` and `consumer` fields.
type RedirectPluginInput struct {
	// Whether the plugin is applied.
	Enabled      *bool             `json:"enabled,omitempty"`
	ID           *string           `json:"id,omitempty"`
	InstanceName *string           `json:"instance_name,omitempty"`
	name         string            `const:"redirect" json:"name"`
	Ordering     map[string]string `json:"ordering,omitempty"`
	// An optional set of strings associated with the Plugin for grouping and filtering.
	Tags   []string             `json:"tags,omitempty"`
	Config RedirectPluginConfig `json:"config"`
	// If set, the plugin will activate only for requests where the specified has been authenticated. (Note that some plugins can not be restricted to consumers this way.). Leave unset for the plugin to activate regardless of the authenticated Consumer.
	Consumer *RedirectPluginConsumer `json:"consumer"`
	// If set, the plugin will activate only for requests where the specified consumer group has been authenticated. (Note that some plugins can not be restricted to consumers groups this way.). Leave unset for the plugin to activate regardless of the authenticated Consumer Groups
	ConsumerGroup *RedirectPluginConsumerGroup `json:"consumer_group"`
	// A set of strings representing HTTP protocols.
	Protocols []RedirectPluginProtocols `json:"protocols,omitempty"`
	// If set, the plugin will only activate when receiving requests via the specified route. Leave unset for the plugin to activate regardless of the route being used.
	Route *RedirectPluginRoute `json:"route"`
	// If set, the plugin will only activate when receiving requests via one of the routes belonging to the specified Service. Leave unset for the plugin to activate regardless of the Service being matched.
	Service *RedirectPluginService `json:"service"`
}

func (r RedirectPluginInput) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(r, "", false)
}

func (r *RedirectPluginInput) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &r, "", false, false); err != nil {
		return err
	}
	return nil
}

func (o *RedirectPluginInput) GetEnabled() *bool {
	if o == nil {
		return nil
	}
	return o.Enabled
}

func (o *RedirectPluginInput) GetID() *string {
	if o == nil {
		return nil
	}
	return o.ID
}

func (o *RedirectPluginInput) GetInstanceName() *string {
	if o == nil {
		return nil
	}
	return o.InstanceName
}

func (o *RedirectPluginInput) GetName() string {
	return "redirect"
}

func (o *RedirectPluginInput) GetOrdering() map[string]string {
	if o == nil {
		return nil
	}
	return o.Ordering
}

func (o *RedirectPluginInput) GetTags() []string {
	if o == nil {
		return nil
	}
	return o.Tags
}

func (o *RedirectPluginInput) GetConfig() RedirectPluginConfig {
	if o == nil {
		return RedirectPluginConfig{}
	}
	return o.Config
}

func (o *RedirectPluginInput) GetConsumer() *RedirectPluginConsumer {
	if o == nil {
		return nil
	}
	return o.Consumer
}

func (o *RedirectPluginInput) GetConsumerGroup() *RedirectPluginConsumerGroup {
	if o == nil {
		return nil
	}
	return o.ConsumerGroup
}

func (o *RedirectPluginInput) GetProtocols() []RedirectPluginProtocols {
	if o == nil {
		return nil
	}
	return o.Protocols
}

func (o *RedirectPluginInput) GetRoute() *RedirectPluginRoute {
	if o == nil {
		return nil
	}
	return o.Route
}

func (o *RedirectPluginInput) GetService() *RedirectPluginService {
	if o == nil {
		return nil
	}
	return o.Service
}
