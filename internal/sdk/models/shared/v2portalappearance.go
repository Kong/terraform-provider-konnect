// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package shared

// V2PortalAppearance - Update a portal's appearance settings
type V2PortalAppearance struct {
	// Select a pre-existing default theme or specify 'custom' to use custom_theme variables.
	ThemeName *V2PortalTheme `json:"theme_name,omitempty"`
	// Groups of variables for configuring visual details of the portal user interface. Set theme_name to 'custom' to use custom values for theme variables.
	CustomTheme *V2NullableAppearanceThemeVariables `json:"custom_theme,omitempty"`
	// Font selections to render text in the portal user interface. Must set use_custom_fonts to true to enable using custom font values.
	CustomFonts *V2NullableAppearanceFonts `json:"custom_fonts,omitempty"`
	// If true, fonts in custom_fonts will be used over the theme's default fonts
	UseCustomFonts *bool `json:"use_custom_fonts,omitempty"`
	// Values to display for customizable text in the portal user interface
	Text *V2NullableAppearanceTextVariables `json:"text,omitempty"`
	// A collection of binary image data to customize images in the portal
	Images *V2AppearanceImages `json:"images,omitempty"`
}

func (o *V2PortalAppearance) GetThemeName() *V2PortalTheme {
	if o == nil {
		return nil
	}
	return o.ThemeName
}

func (o *V2PortalAppearance) GetCustomTheme() *V2NullableAppearanceThemeVariables {
	if o == nil {
		return nil
	}
	return o.CustomTheme
}

func (o *V2PortalAppearance) GetCustomFonts() *V2NullableAppearanceFonts {
	if o == nil {
		return nil
	}
	return o.CustomFonts
}

func (o *V2PortalAppearance) GetUseCustomFonts() *bool {
	if o == nil {
		return nil
	}
	return o.UseCustomFonts
}

func (o *V2PortalAppearance) GetText() *V2NullableAppearanceTextVariables {
	if o == nil {
		return nil
	}
	return o.Text
}

func (o *V2PortalAppearance) GetImages() *V2AppearanceImages {
	if o == nil {
		return nil
	}
	return o.Images
}
