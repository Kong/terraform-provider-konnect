...
func (k *KafkaUpstreamPluginConfluent) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &k, "", false, []string{"authentication", "key_schema", "value_schema"}); err != nil {
		return err
	}
	return nil
}
...
func (k *KafkaUpstreamPluginConfig) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &k, "", false, []string{"allowed_topics", "authentication", "bootstrap_servers", "message_by_lua_functions", "schema_registry", "security", "topic"}); err != nil {
		return err
	}
	return nil
}
