// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"github.com/kong/terraform-provider-konnect/v3/internal/sdk/internal/utils"
	"github.com/kong/terraform-provider-konnect/v3/internal/sdk/models/shared"
	"net/http"
)

type FetchAPISpecRequest struct {
	// The UUID API identifier
	APIID string `pathParam:"style=simple,explode=false,name=apiId"`
	// The API specification identifier
	SpecID string `pathParam:"style=simple,explode=false,name=specId"`
}

func (f FetchAPISpecRequest) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(f, "", false)
}

func (f *FetchAPISpecRequest) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &f, "", false, []string{"apiId", "specId"}); err != nil {
		return err
	}
	return nil
}

func (f *FetchAPISpecRequest) GetAPIID() string {
	if f == nil {
		return ""
	}
	return f.APIID
}

func (f *FetchAPISpecRequest) GetSpecID() string {
	if f == nil {
		return ""
	}
	return f.SpecID
}

type FetchAPISpecResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// API specification (OpenAPI or AsyncAPI)
	APISpecResponse *shared.APISpecResponse
	// ApiUnauthorized
	UnauthorizedError *shared.UnauthorizedError
	// ApiForbidden
	ForbiddenError *shared.ForbiddenError
	// Not Found
	NotFoundError *shared.NotFoundError
}

func (f FetchAPISpecResponse) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(f, "", false)
}

func (f *FetchAPISpecResponse) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &f, "", false, []string{"ContentType", "StatusCode", "RawResponse"}); err != nil {
		return err
	}
	return nil
}

func (f *FetchAPISpecResponse) GetContentType() string {
	if f == nil {
		return ""
	}
	return f.ContentType
}

func (f *FetchAPISpecResponse) GetStatusCode() int {
	if f == nil {
		return 0
	}
	return f.StatusCode
}

func (f *FetchAPISpecResponse) GetRawResponse() *http.Response {
	if f == nil {
		return nil
	}
	return f.RawResponse
}

func (f *FetchAPISpecResponse) GetAPISpecResponse() *shared.APISpecResponse {
	if f == nil {
		return nil
	}
	return f.APISpecResponse
}

func (f *FetchAPISpecResponse) GetUnauthorizedError() *shared.UnauthorizedError {
	if f == nil {
		return nil
	}
	return f.UnauthorizedError
}

func (f *FetchAPISpecResponse) GetForbiddenError() *shared.ForbiddenError {
	if f == nil {
		return nil
	}
	return f.ForbiddenError
}

func (f *FetchAPISpecResponse) GetNotFoundError() *shared.NotFoundError {
	if f == nil {
		return nil
	}
	return f.NotFoundError
}
