...
// CentrallyManagedKeys - Centrally managed consumers use key-auth mechanism to authenticate consumers. Keys refer to the API key credential used by the consumer to authenticate to your services.
type CentrallyManagedKeys struct {
	rootSDK          *Konnect
	sdkConfiguration config.SDKConfiguration
	hooks            *hooks.Hooks
}

func newCentrallyManagedKeys(rootSDK *Konnect, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *CentrallyManagedKeys {
	return &CentrallyManagedKeys{
		rootSDK:          rootSDK,
...
// CreateConsumerKey - Create a key
...
func (s *CentrallyManagedKeys) CreateConsumerKey(ctx context.Context, request operations.CreateConsumerKeyRequest, opts ...operations.Option) (*operations.CreateConsumerKeyResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/v1/realms/{realmId}/consumers/{consumerId}/keys", request, nil)
	if err != nil {
	...
	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, false, false, "CreateAPIKeyPayload", "json", `request:"mediaType=application/json"`)
	if err != nil {
		return nil, err
	}
	...
	res := &operations.CreateConsumerKeyResponse{
	...
	case httpRes.StatusCode == 201:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out shared.CreateAPIKeyResult
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.CreateAPIKeyResult = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
...
// GetConsumerKey - Get a key
// Returns the key entity. This operation does **NOT** return the key itself. The key is only available at the time of creation.
func (s *CentrallyManagedKeys) GetConsumerKey(ctx context.Context, request operations.GetConsumerKeyRequest, opts ...operations.Option) (*operations.GetConsumerKeyResponse, error) {
	o := operations.Options{}
	supportedOptions := []string{
		operations.SupportedOptionTimeout,
		operations.SupportedOptionAcceptHeaderOverride,
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/v1/realms/{realmId}/consumers/{consumerId}/keys/{keyId}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.GetConsumerKeyResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: httpRes.Header.Get("Content-Type"),
		RawResponse: httpRes,
	}
...
// DeleteConsumerKey - Delete a key
...
func (s *CentrallyManagedKeys) DeleteConsumerKey(ctx context.Context, request operations.DeleteConsumerKeyRequest, opts ...operations.Option) (*operations.DeleteConsumerKeyResponse, error) {
	o := operations.Options{}
	supportedOptions := []string{
		operations.SupportedOptionTimeout,
	}
	...
	res := &operations.DeleteConsumerKeyResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: httpRes.Header.Get("Content-Type"),
		RawResponse: httpRes,
	}
