...
func (r *MeshGlobalRateLimitResourceModel) ToSharedMeshGlobalRateLimitItem() *shared.MeshGlobalRateLimitItem {
	...
	for _, fromItem := range r.Spec.From {
		...
		kind1 := new(shared.MeshGlobalRateLimitItemSpecKind)
	...
	if r.Spec.TargetRef != nil {
		kind2 := new(shared.MeshGlobalRateLimitItemKind)
	...
	for _, toItem := range r.Spec.To {
		...
		kind4 := new(shared.MeshGlobalRateLimitItemSpecToKind)
	...
	if resp != nil {
		...
		for fromCount, fromItem := range resp.Spec.From {
			...
			if fromItem.Default == nil {
				...
				from1.Default.Backend.RateLimitService.Timeout = types.StringValue(fromItem.Default.Backend.RateLimitService.Timeout)
				from1.Default.Backend.RateLimitService.URL = types.StringValue(fromItem.Default.Backend.RateLimitService.URL)
		...
		for toCount, toItem := range resp.Spec.To {
			...
			if toItem.Default == nil {
				...
				to1.Default.Backend.RateLimitService.Timeout = types.StringValue(toItem.Default.Backend.RateLimitService.Timeout)
				to1.Default.Backend.RateLimitService.URL = types.StringValue(toItem.Default.Backend.RateLimitService.URL)
