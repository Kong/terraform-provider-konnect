// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package types

import (
	"github.com/hashicorp/terraform-plugin-framework/types"
)

type PartialModelOptions struct {
	AnthropicVersion     types.String                      `tfsdk:"anthropic_version"`
	AzureAPIVersion      types.String                      `tfsdk:"azure_api_version"`
	AzureDeploymentID    types.String                      `tfsdk:"azure_deployment_id"`
	AzureInstance        types.String                      `tfsdk:"azure_instance"`
	Bedrock              *PartialEmbeddingsBedrock         `tfsdk:"bedrock"`
	CacheReadCost        types.Float64                     `tfsdk:"cache_read_cost"`
	CacheWriteCost       types.Float64                     `tfsdk:"cache_write_cost"`
	CacheWriteCostList   []PartialModelCacheWriteCostList  `tfsdk:"cache_write_cost_list"`
	Cohere               *PartialModelCohere               `tfsdk:"cohere"`
	ContextWindowFactor  []PartialModelContextWindowFactor `tfsdk:"context_window_factor"`
	Dashscope            *PartialModelDashscope            `tfsdk:"dashscope"`
	Databricks           *PartialModelDatabricks           `tfsdk:"databricks"`
	EmbeddingsDimensions types.Int64                       `tfsdk:"embeddings_dimensions"`
	Gemini               *PartialModelGemini               `tfsdk:"gemini"`
	Huggingface          *PartialEmbeddingsHuggingface     `tfsdk:"huggingface"`
	InputCost            types.Float64                     `tfsdk:"input_cost"`
	Llama2Format         types.String                      `tfsdk:"llama2_format"`
	MaxTokens            types.Int64                       `tfsdk:"max_tokens"`
	MistralFormat        types.String                      `tfsdk:"mistral_format"`
	OutputCost           types.Float64                     `tfsdk:"output_cost"`
	ServiceTierFactor    []PartialModelServiceTierFactor   `tfsdk:"service_tier_factor"`
	Temperature          types.Float64                     `tfsdk:"temperature"`
	TopK                 types.Int64                       `tfsdk:"top_k"`
	TopP                 types.Float64                     `tfsdk:"top_p"`
	UpstreamPath         types.String                      `tfsdk:"upstream_path"`
	UpstreamURL          types.String                      `tfsdk:"upstream_url"`
}
