...
func (r *GatewayPluginAcmeResourceModel) ToSharedCreateAcmePlugin() *shared.CreateAcmePlugin {
	...
	if r.Config != nil {
		...
		var accountKey *shared.CreateAcmePluginAccountKey
		if r.Config.AccountKey != nil {
			...
			accountKey = &shared.CreateAcmePluginAccountKey{
				KeyID:  keyID,
				KeySet: keySet,
			}
		}
		...
		certType := new(shared.CreateAcmePluginCertType)
		if !r.Config.CertType.IsUnknown() && !r.Config.CertType.IsNull() {
			*certType = shared.CreateAcmePluginCertType(r.Config.CertType.ValueString())
		} else {
			certType = nil
		}
		...
		rsaKeySize := new(shared.CreateAcmePluginRsaKeySize)
		if !r.Config.RsaKeySize.IsUnknown() && !r.Config.RsaKeySize.IsNull() {
			*rsaKeySize = shared.CreateAcmePluginRsaKeySize(r.Config.RsaKeySize.ValueInt64())
		} else {
			rsaKeySize = nil
		}
		storage := new(shared.CreateAcmePluginStorage)
		if !r.Config.Storage.IsUnknown() && !r.Config.Storage.IsNull() {
			*storage = shared.CreateAcmePluginStorage(r.Config.Storage.ValueString())
		} else {
			storage = nil
		}
		var storageConfig *shared.CreateAcmePluginStorageConfig
		if r.Config.StorageConfig != nil {
			var consul *shared.CreateAcmePluginConsul
			if r.Config.StorageConfig.Consul != nil {
				...
				consul = &shared.CreateAcmePluginConsul{
					Host:    host,
					HTTPS:   https,
					KvPath:  kvPath,
					Port:    port,
			...
			if r.Config.StorageConfig.Redis != nil {
				...
				var extraOptions *shared.CreateAcmePluginExtraOptions
				if r.Config.StorageConfig.Redis.ExtraOptions != nil {
					...
					extraOptions = &shared.CreateAcmePluginExtraOptions{
						Namespace: namespace,
						ScanCount: scanCount,
					}
				}
			...
			var shm *shared.CreateAcmePluginShm
			if r.Config.StorageConfig.Shm != nil {
				...
				shm = &shared.CreateAcmePluginShm{
					ShmName: shmName,
				}
			}
			var vault *shared.CreateAcmePluginVault
			if r.Config.StorageConfig.Vault != nil {
				authMethod := new(shared.CreateAcmePluginAuthMethod)
				if !r.Config.StorageConfig.Vault.AuthMethod.IsUnknown() && !r.Config.StorageConfig.Vault.AuthMethod.IsNull() {
					*authMethod = shared.CreateAcmePluginAuthMethod(r.Config.StorageConfig.Vault.AuthMethod.ValueString())
				} else {
					authMethod = nil
				}
				authPath := new(string)
			...
			storageConfig = &shared.CreateAcmePluginStorageConfig{
				Consul: consul,
				Kong:   kong,
				Redis:  redis,
				Shm:    shm,
	...
	var ordering *shared.CreateAcmePluginOrdering
	if r.Ordering != nil {
		var after *shared.CreateAcmePluginAfter
		if r.Ordering.After != nil {
			var access []string = []string{}
			for _, accessItem := range r.Ordering.After.Access {
				access = append(access, accessItem.ValueString())
			}
			after = &shared.CreateAcmePluginAfter{
				Access: access,
			}
		}
		var before *shared.CreateAcmePluginBefore
		if r.Ordering.Before != nil {
			var access1 []string = []string{}
			for _, accessItem1 := range r.Ordering.Before.Access {
				access1 = append(access1, accessItem1.ValueString())
			}
			before = &shared.CreateAcmePluginBefore{
				Access: access1,
			}
		}
		ordering = &shared.CreateAcmePluginOrdering{
			After:  after,
			Before: before,
		}
	}
